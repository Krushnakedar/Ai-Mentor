...
const DiscussionsPage = () => {
  ...
  const fetchCoursePosts = useCallback(
    async (sort) => {
      ...
      try {
        ...
        const res = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/apicommunity/course-posts${q}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
  ...
  const fetchGlobalPosts = useCallback(
    async (cat, sort) => {
      ...
      try {
        ...
        const res = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/api/community/global?${params}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
  ...
  const doAction = async (id, action, body) => {
    ...
    const res = await fetch(
      `${import.meta.env.VITE_API_BASE_URL}/api/community/${id}/${action}`,
      {
        method,
        headers: authHeaders(),
  ...
  const handleReport = async () => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/community/${reportModal.postId}/report`,
        {
          method: "POST",
          headers: authHeaders(),
          body: JSON.stringify({
  ...
  const handleEditReply = async (postId, replyId, newText) => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/community/${postId}/reply/${replyId}`,
        {
          method: "PUT",
          headers: authHeaders(),
  ...
  const handleEditPost = async (postId, newContent) => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/community/${postId}`,
        {
          method: "PUT",
          headers: authHeaders(),
  ...
  const fetchReports = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/community/reports`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const executeModeration = async (reportId, action) => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/community/reports/${reportId}`,
        {
          method: "PUT",
          headers: authHeaders(),
  ...
  const handleUnhide = async (postId, replyId = null) => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/community/${postId}/unhide`,
        {
          method: "PUT",
          headers: authHeaders(),
            ...
            <div className="bg-card border border-border rounded-xl shadow-2xl w-full max-w-sm">
              ...
              <div className="px-4 py-5 text-center space-y-3">
                ...
                <p className="text-xs text-muted">
                  Your report helps us protect the community from harmful
                  content.
                </p>
                <p className="text-xs text-muted">
                ...
                <div className="text-left mt-3">
                  ...
                  <div className="flex items-start gap-2.5 bg-canvas-alt rounded-lg p-2.5">
                    ...
                    <p className="text-[11px] text-muted leading-relaxed">
                      If this commenter has serious or repeated violations, we
                      may temporarily restrict their ability to leave comments.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-card border border-border rounded-xl shadow-2xl w-full max-w-sm">
              <div className="px-5 py-6 text-center space-y-3">
                ...
                <p className="text-sm text-muted">
                  You have already reported this content. Our team will review
                  it shortly.
                </p>
              </div>
